...
export default function AgentThinking({ trace, timings, meta }) {
  ...
  const { thinking_steps, iterations, total_simulations, total_tokens, parsed_constraints } = trace;
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-purple-200">
              {iterations.length} iterations • {total_simulations} simulations • {total_tokens} tokens
            </p>
          </div>
        </div>
      ...
      {expanded && (
        <div className="p-6 space-y-6">
          ...
          <div className="bg-gray-800 rounded-xl p-4 border border-gray-700">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
              <div className="bg-gray-700 p-3 rounded-lg">
                ...
                <div className="text-xl font-bold text-blue-400">L{parsed_constraints.base_lap}</div>
              </div>
              <div className="bg-gray-700 p-3 rounded-lg">
                <div className="text-gray-400 text-xs">Gap to Competitor</div>
                <div className={`text-xl font-bold ${parsed_constraints.base_target_gap_s >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                  {parsed_constraints.base_target_gap_s >= 0 ? '+' : ''}{parsed_constraints.base_target_gap_s?.toFixed(2)}s
                </div>
              </div>
              <div className="bg-gray-700 p-3 rounded-lg">
              ...
              <div className="bg-gray-700 p-3 rounded-lg">
                ...
                <div className="text-xl font-bold text-purple-400">{parsed_constraints.current_tire_age} laps</div>
              </div>
            </div>
            {parsed_constraints.objective && (
              <div className="mt-3 p-3 bg-blue-900/30 rounded-lg border border-blue-700">
                ...
                <div className="text-sm font-semibold">{parsed_constraints.objective}</div>
              </div>
            )}
          </div>
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {thinking_steps.map((step, idx) => (
                <div key={idx} className="flex items-start gap-3 text-sm">
                  <span className="text-gray-500 font-mono text-xs mt-0.5">{String(idx + 1).padStart(2, '0')}</span>
          ...
          {iterations.length > 0 && (
            <div className="bg-gray-800 rounded-xl p-4 border border-gray-700">
              <h4 className="text-lg font-semibold mb-3 flex items-center gap-2">
                <span>🔄</span> Iterative Refinement ({iterations.length} rounds)
              </h4>
              <div className="space-y-4">
                {iterations.map((iter, idx) => (
                  <div key={idx} className="bg-gray-900 rounded-lg p-4 border border-gray-700">
                    <div className="flex items-center justify-between mb-3">
                      <div className="text-sm font-bold text-purple-400">Iteration {iter.iteration}</div>
                          ...
                          >
                            <div className="flex items-center justify-between">
                              <span className="font-semibold">
                                L{res.candidate?.pit_lap} {res.candidate?.compound?.toUpperCase()}
                              </span>
                              {isTop && <span className="text-green-400 text-xs">✓ Best</span>}
                            </div>
                            <div className={`text-lg font-bold mt-1 ${
                              res.median_gap_after_5_laps >= 0 ? 'text-green-400' : 'text-orange-400'
                            }`}>
                              {res.median_gap_after_5_laps >= 0 ? '+' : ''}{res.median_gap_after_5_laps?.toFixed(2)}s
                            </div>
                          </div>
                        );
                      })}
          ...
          <div className="bg-gray-800 rounded-xl p-4 border border-gray-700">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3 text-sm">
              <div className="bg-gray-900 p-3 rounded-lg border border-gray-700">
                ...
                <div className="text-sm font-bold text-cyan-400">{meta?.provider || 'N/A'}</div>
              </div>
              <div className="bg-gray-900 p-3 rounded-lg border border-gray-700">
                <div className="text-gray-400 text-xs">Model</div>
                <div className="text-sm font-bold text-purple-400">
                  {meta?.planner_model?.split('-').slice(0, 3).join('-') || 'N/A'}
                </div>
              </div>
              <div className="bg-gray-900 p-3 rounded-lg border border-gray-700">
                <div className="text-gray-400 text-xs">Total Tokens</div>
                <div className="text-sm font-bold text-yellow-400">{total_tokens?.toLocaleString() || 0}</div>
              </div>
              <div className="bg-gray-900 p-3 rounded-lg border border-gray-700">
                <div className="text-gray-400 text-xs">Planning Time</div>
                <div className="text-sm font-bold text-green-400">{timings?.planner_s?.toFixed(2)}s</div>
              </div>
              <div className="bg-gray-900 p-3 rounded-lg border border-gray-700">
                <div className="text-gray-400 text-xs">Total Time</div>
                <div className="text-sm font-bold text-blue-400">{timings?.total_s?.toFixed(2)}s</div>
              </div>
            </div>
          </div>
        </div>
