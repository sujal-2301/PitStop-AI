...
export default function OutcomeBanner({ simResult, explanation }) {
  ...
  const secondBest = otherCandidates.length > 0 
        ...
        <div className="p-8 text-white">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <div className="bg-white/10 backdrop-blur rounded-xl p-4 border border-white/20">
              <div className="text-xs text-green-100 mb-1 uppercase tracking-wide">Time Advantage</div>
              <div className={`text-3xl font-bold ${finalGap >= 0 ? 'text-white' : 'text-yellow-200'}`}>
                {finalGap >= 0 ? '+' : ''}{finalGap.toFixed(2)}s
              </div>
              <div className="text-xs text-green-100 mt-1">
                {finalGap >= 0 ? 'You\'ll be ahead' : 'You\'ll be behind'} by lap {simResult.base_lap + 10}
              </div>
            </div>

            {/* Improvement */}
            <div className="bg-white/10 backdrop-blur rounded-xl p-4 border border-white/20">
              <div className="text-xs text-green-100 mb-1 uppercase tracking-wide">Net Change</div>
              <div className={`text-3xl font-bold ${improvement >= 0 ? 'text-white' : 'text-yellow-200'}`}>
                {improvement >= 0 ? '+' : ''}{improvement.toFixed(2)}s
              </div>
              <div className="text-xs text-green-100 mt-1">
                {improvement >= 0 ? '📈 Improved' : '📉 Declined'} from starting position
              </div>
            </div>

            {/* Confidence */}
            <div className="bg-white/10 backdrop-blur rounded-xl p-4 border border-white/20">
              <div className="text-xs text-green-100 mb-1 uppercase tracking-wide">Confidence</div>
          ...
          <div className="flex flex-wrap gap-4 text-sm">
            {deltaVsNext !== 0 && (
              <div className="bg-white/10 backdrop-blur px-4 py-2 rounded-lg border border-white/20">
                ...
                <span className="font-bold text-white ml-2">+{deltaVsNext.toFixed(2)}s</span>
              </div>
            )}
            {breakeven && (
              <div className="bg-white/10 backdrop-blur px-4 py-2 rounded-lg border border-white/20">
        ...
        <div className="bg-white/5 backdrop-blur px-8 py-6 border-t border-white/10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-xs text-green-100">Now (Lap {simResult.base_lap})</div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-xs text-green-100">Pit Stop (Lap {pitLap})</div>
                <div className="text-lg font-bold text-white">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className={`text-lg font-bold ${finalGap >= 0 ? 'text-white' : 'text-yellow-200'}`}>
                  {finalGap >= 0 ? '+' : ''}{finalGap.toFixed(2)}s
                </div>
                <div className="text-xs text-green-200">
                  {finalGap >= 0 ? 'Ahead' : 'Behind'} by lap {simResult.base_lap + 10}
                </div>
              </div>
            </div>
          </div>
